...
void Adafruit_LEDBackpack::setBrightness(uint8_t b) {
  if (b > 15) b = 15;
  I2c.write(i2c_addr,(uint8_t)(HT16K33_CMD_BRIGHTNESS | b));
}
...
void Adafruit_LEDBackpack::begin(uint8_t _addr = 0x70) {
  ...
  I2c.setSpeed(true); // Enable Highspeed
  I2c.timeOut(10);
  I2c.begin();
...
void Adafruit_LEDBackpack::writeDisplay(void) {
  I2c.write(i2c_addr,(uint8_t)0x00); // start at address $00

  
  for (uint8_t i=0; i<8; i++) {
    I2c.write(i2c_addr,2*i,(uint8_t)(displaybuffer[i] & 0xFF));    
    I2c.write(i2c_addr,(2*i)+1,(uint8_t)(displaybuffer[i] >> 8));    
  }
}
...
void Adafruit_7segment::writeColon(void) {
    I2c.write(i2c_addr,(uint8_t)0x04); // start at address $02
    
    I2c.write(i2c_addr,0x04,(uint8_t)(displaybuffer[2] & 0xFF));
    I2c.write(i2c_addr,0x05,(uint8_t)(displaybuffer[2] >> 8));
}
...
{ 
  ...
  if (toIntFactor < 1) {
    ...
    {
      int8_t myDisplayPos = 0;
      
      for(uint8_t i = 0; displayNumber || i <= fracDigits; ++i) {
        boolean displayDecimal = (fracDigits != 0 && i == fracDigits);
        myDisplayPos = 4 - displayPos--;
        writeDigitNum(myDisplayPos, displayNumber % base, displayDecimal);
